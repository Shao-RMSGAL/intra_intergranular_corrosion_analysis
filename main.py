...
try:
    from eds_cython import (
        ...
        generate_intragranular_mask_fast
    )
...
class EDSAnalyzer:  # {{{
    def __init__(self, root):
        ...
        self.intragranular_mask = np.zeros((0, 0), dtype=np.uint8)
    ...
    def setup_main_tab(self):
        ...
        self.eds_threshold_entry = tk.Entry(
            eds_thresh_frame, textvariable=self.eds_threshold_var, width=10, state=tk.DISABLED)
    ...
    #      eds_label.pack(side=tk.LEFT)
    #      self.eds_threshold_var = tk.StringVar(value="5")
    #      self.eds_threshold_entry = tk.Entry(eds_thresh_frame, textvariable=self.eds_threshold_var, width=10, state=tk.DISABLED) self.eds_threshold_entry.pack(side=tk.LEFT, padx=(10, 0))
...
# Add these methods after generate_eds_mask method:
    ...
    def generate_intragranular_mask(self, threshold_percent):
        ...
        if CYTHON_AVAILABLE:
            # Use Cython optimized version
            eds_gray = cv2.cvtColor(self.eds_original, cv2.COLOR_BGR2GRAY)
            intergranular_mask_uint8 = self.eds_mask.astype(np.uint8) * 255
            exclusion_mask_uint8 = self.exclusion_mask.astype(
                np.uint8) * 255 if self.exclusion_mask.shape != (0, 0) else np.zeros((0, 0), dtype=np.uint8)

            self.intragranular_mask = generate_intragranular_mask_fast(
                eds_gray, intergranular_mask_uint8, exclusion_mask_uint8, self.max_eds, threshold_percent
            ).astype(bool)
        else:
            # Fall back to original Python implementation
            self._generate_intragranular_mask_python(threshold_percent)

    def _generate_intragranular_mask_python(self, threshold_percent):
        """Original Python implementation for intragranular mask"""
        self.intragranular_mask = np.zeros(
            self.eds_original.shape[:2], dtype=bool)
        ...
        for y in range(eds_gray.shape[0]):
            for x in range(eds_gray.shape[1]):
                if (eds_gray[y, x] < threshold_value and
                    not self.eds_mask[y, x] and
                        (self.exclusion_mask.shape == (0, 0) or not self.exclusion_mask[y, x])):
                    self.intragranular_mask[y, x] = True
